<script>
  ...
  function setUiCapabilites() {
    return new Promise((accept, reject) => {
      const url = `${drupalSettings.project_browser.origin_url}/admin/modules/project_browser/install-readiness`;
      fetch(url)
        .then(async (response) => {
          if (!response.ok) {
  ...
  <ProjectGrid {loading} {rows} {pageIndex} {pageSize} let:rows>
    <div slot="top">
      ...
      <!-- If UI installs are enabled, but the site configuration does not them,
           ...
           installs to work. -->
      {#if $uiCapabilities.pm_validation_error && typeof $uiCapabilities.pm_validation_error === 'string' && drupalSettings.project_browser.modules.package_manager && drupalSettings.project_browser.ui_install}
